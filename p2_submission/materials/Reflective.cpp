...
Reflective::Reflective() {
    ambient_brdf.set_kd(0.1f);  ambient_brdf.set_cd(RGBColor(1,1,1));
    diffuse_brdf.set_kd(0.4f);  diffuse_brdf.set_cd(RGBColor(1,1,1));
    glossy_brdf.set_ks(0.3f);   glossy_brdf.set_cs(RGBColor(1,1,1));
    glossy_brdf.set_exp(50.0f);
    specular_brdf.set_kr(0.5f); specular_brdf.set_cr(RGBColor(1,1,1));
}

Reflective::Reflective(float ka, float kd, float ks,
                        float kr, const RGBColor& c, float exp) {
    ambient_brdf.set_kd(ka);  ambient_brdf.set_cd(c);
    diffuse_brdf.set_kd(kd);  diffuse_brdf.set_cd(c);
    glossy_brdf.set_ks(ks);   glossy_brdf.set_cs(RGBColor(1,1,1));
    glossy_brdf.set_exp(exp);
    specular_brdf.set_kr(kr); specular_brdf.set_cr(RGBColor(1,1,1));
}
...
void Reflective::set_cd(const RGBColor& c) { ambient_brdf.set_cd(c); diffuse_brdf.set_cd(c); }
