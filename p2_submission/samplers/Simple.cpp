...
std::vector<Ray> Simple::get_rays(int px, int py) const {
    // Compute the center of pixel (px, py) in world coordinates.
    // px goes 0..hres-1 (left to right), py goes 0..vres-1 (top to bottom).
    float hres = (float)viewplane_ptr->hres;
    float vres = (float)viewplane_ptr->vres;

    // Interpolate across the view plane
    float u = (px + 0.5f) / hres;  // [0,1] left to right
    float v = (py + 0.5f) / vres;  // [0,1] top to bottom

    Point3D tl = viewplane_ptr->top_left;
    Point3D br = viewplane_ptr->bottom_right;

    // World-space pixel center
